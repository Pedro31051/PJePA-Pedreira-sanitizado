...
def zero_retention_enabled() -> bool:
    """Process data must never enter a persistent ADK session by default."""
    return os.environ.get("PJE_ZERO_RETENTION", "1").strip().casefold() not in {
        "0",
        "false",
        "no",
    }
...
def get_artifact_service():
    """Keep transient agent artifacts in memory; never persist process content."""
